...
def fit_lstm(train, batch_size, nb_epoch, neurons):
	...
	# nb_epoch 置1，用epochs循环
	nb_epoch = 1
	for _ in range(nb_epoch):
		        # shuffle=False是不混淆数据顺序
		        #model.fit(X, y, epochs=10,validation_split=0.33, batch_size=batch_size, verbose=1, shuffle=False)
		        # 添加验证集验证数据，取最后30条数据作为验证数据
		History = model.fit(
		    X[:-30],
		    y[:-30],
		    epochs=1000,
		    validation_data=(X[-30:], y[-30:]),
...
lstm_model = fit_lstm(train_scaled, 1, 3000, 8)
# 重构输入数据的形状，
print(train_scaled)
